describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 1; i < 20; i++){ // test 19 more times
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let plainValue;
    for (let i = 1; i < 20; i++){ // first item already clicked by previous test
      ...
      plainValue = await innerText.jsonValue();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < 20; i++){
      ...
      let plainValue = await buttonText.jsonValue();
      expect(plainValue).toBe('Remove from Cart');
    }
    const cart = await page.$('#cart-count');
    const cartText = await cart.getProperty('innerText');
    let plainValue = await cartText.jsonValue();
    expect(plainValue).toBe('20');
  }, 10000);
